...
function enter(px, reason, isPaper) {
  ...
  const safeTp = Math.max(px * (1 + S.tpPct/100), minTpPx(px, amt));
  const tp   = parseFloat(safeTp.toFixed(4));
  const sl   = parseFloat((px * (1 - S.slPct/100)).toFixed(4));
  const o = {
  ...
  if (isPaper) {
    S.papOrders.push(o);
    log('PAPER BUY @ $' + px.toFixed(2) + ' TP=' + tp.toFixed(2) + ' SL=' + sl.toFixed(2), 'buy');
  } else {
    S.liveOrders.push(o);
    log('LIVE BUY @ $' + px.toFixed(2) + ' TP=' + tp.toFixed(2) + ' SL=' + sl.toFixed(2), 'buy');
...
// ── FUTURES FEE MATH ──────────────────────────────────────────────────────────
function futFee(entryPx, exitPx, margin, lev) {
...
function futBE(entryPx, margin, lev) {
  const notional  = margin * lev;
  const contracts = notional / entryPx;
...
function futEnter(px, reason, isPaper) {
  ...
  const notional = margin * lev;
  const bePx     = parseFloat(futBE(px, margin, lev).toFixed(4));
  const wantTp   = px * (1 + S.futTpPct / 100);
  const minTp    = bePx * 1.001;
  const tp       = parseFloat(Math.max(wantTp, minTp).toFixed(4));
  const tpDist   = tp - px;
  const sl       = parseFloat(Math.max(px * (1 - S.futSlPct/100), px - tpDist * 0.55).toFixed(4));
  const expNet   = futFee(px, tp, margin, lev).net;
  const o = {
    id: Date.now() + (isPaper?1:0), status:'open', isPaper, isFutures:true,
    direction:'LONG', entryPx:px, margin, leverage:lev, notional, tp, sl, bePx,
    beStopMoved:false, peakPx:px, peakNet:0,
    openAt:new Date().toISOString().slice(11,19), reason
  };
  log((isPaper?'FUT-PAPER':'FUT-LIVE') + ' LONG @ $' + px.toFixed(2) +
      ' margin=$' + margin.toFixed(2) + ' ' + lev + 'x | TP=$' + tp.toFixed(2) +
      ' SL=$' + sl.toFixed(2) + ' | exp=+$' + expNet.toFixed(4), 'buy');
  if (isPaper) S.futPapOrders.push(o);
  else { S.futOrders.push(o); futPlaceOrder('open_long', margin, lev, px); }
}
...
async function onFutTick(px) {
  ...
  if (futTicks % 20 === 0) log('[FUT T' + futTicks + '] $' + px.toFixed(2) + ' sig=' + sig.signal, 'info');

  if (S.aiMode === 'ai-only') {
    if (S.aiKey) await callAI(px, true);
    const aiOk = checkAI(true);
    if (!aiOk.ok) return;
  } else {
    if (!sig.signal) { if (S.aiKey) callAI(px, true); return; }
    if (S.aiKey && S.aiMode !== 'off') await callAI(px, true);
    const aiOk = checkAI(true);
    if (!aiOk.ok) { if (aiOk.reason) log('AI blocked fut: ' + aiOk.reason, 'info'); return; }
  }
  ...
  if (S.futMode === 'live' && S.apiKey && S.apiSecret) {
    ...
    if (liveOpen < S.futMaxPos && S.futT < (S.futMaxDaily||300)) {
      if (S.futCapital / S.futMaxPos >= 5) { futEnter(px, sig.reason, false); }
    }
  }
  S.futLastEntry = now;
}
...
async function callAI(px, isFutures) {
  ...
  p += 'Rules: BUY if RSI<50+dip+bounce. HOLD if RSI>65 or falling. ' + (isFutures?'SHORT if strong downtrend.':'') + '\n';
